...
class VideoGenerator:
    ...
    def __init__(
        ...
        default_model: str = "model1",
        logger: Optional[logging.Logger] = None,
    ):
        self._logger = logger or logging.getLogger("video_generator.generator")
        
        self._models_config = EnvUtils.resolve_api_keys(models_config, logger=self._logger)
        self._current_model_id = default_model
        self._provider_instances: Dict[str, Any] = {}
        
        available_count = sum(
            1 for cfg in self._models_config.values()
        ...
        self._logger.debug(f"[VideoGenerator] 初始化: {total_count} 个模型，{available_count} 个已配置，默认 {default_model}")

    def _get_provider_instance(self, model_id: str):
        """获取模型对应的服务商实例"""
        if model_id not in self._models_config:
            self._logger.error(f"[VideoGenerator] 模型 {model_id} 不存在")
            return None
            
        model_config = self._models_config[model_id]
        
        if not isinstance(model_config, dict):
            self._logger.error(f"[VideoGenerator] 模型 {model_id} 配置格式错误")
            return None
        
        api_key = model_config.get("api_key", "")
        
        if not api_key:
            self._logger.error(f"[VideoGenerator] 模型 {model_id} 未配置 API Key")
            return None
        ...
        if model_id not in self._provider_instances:
            ...
            if not provider_class:
                self._logger.error(f"[VideoGenerator] 不支持的 API 格式: {api_format}")
                return None
            ...
            try:
                ...
                self._logger.info(f"[VideoGenerator] 初始化服务商: {model_id} ({api_format})")
            except Exception as e:
                self._logger.error(f"[VideoGenerator] 初始化服务商失败: {e}")
                return None
    ...
    def switch_model(self, model_id: str) -> bool:
        """切换当前模型"""
        if model_id not in self._models_config:
            self._logger.warning(f"[VideoGenerator] 切换失败: 模型 {model_id} 不存在")
            return False
        
        config = self._models_config[model_id]
        if not isinstance(config, dict) or not config.get("format"):
            self._logger.warning(f"[VideoGenerator] 切换失败: 模型 {model_id} 配置无效")
            return False
        
        if not config.get("api_key"):
            self._logger.warning(f"[VideoGenerator] 切换失败: 模型 {model_id} 未配置 API Key")
            return False
        
        self._current_model_id = model_id
        self._logger.info(f"[VideoGenerator] 已切换到模型: {model_id}")
        return True
    ...
    async def generate_video(
        ...
        **kwargs
        ...
        if use_model_id not in self._models_config:
            ...
            self._logger.error(f"[VideoGenerator] {error_msg}")
        ...
        if (image_url or last_frame_url) and not model_config.get("support_img2video", True):
            self._logger.warning(f"[VideoGenerator] 模型 {use_model_id} 不支持图生视频，将忽略图片")
            image_url = None
            last_frame_url = None
        ...
        try:
            ...
            self._logger.info(f"[VideoGenerator] 提交任务: 模型={use_model_id}, 模式={mode}")
            ...
            self._logger.info(f"[VideoGenerator] 任务创建成功: {task_id}")
            return True, task_id, use_model_id
            
        except Exception as e:
            error_msg = str(e)
            self._logger.error(f"[VideoGenerator] 创建任务失败: {error_msg}")
            import traceback
            traceback.print_exc()
    ...
    async def get_task_status(self, task_id: str, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.debug(f"[VideoGenerator] 任务状态: {task_id} -> {status.get('status')}")
            return status
        except Exception as e:
            self._logger.error(f"[VideoGenerator] 获取状态失败: {e}")
    ...
    async def cancel_task(self, task_id: str, model_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            self._logger.error(f"[VideoGenerator] 取消任务失败: {e}")
            return False, str(e)