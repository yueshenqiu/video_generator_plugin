...
![MaiBot](https://img.shields.io/badge/MaiBot-1.0.0+-orange.svg)
...
| 服务商 | 控制台地址 | 说明 |
|--------|-----------|------|
| 火山引擎 | [console.volcengine.com](https://console.volcengine.com) | 需开通「豆包大模型」服务 |
| 阿里云 | [dashscope.console.aliyun.com](https://dashscope.console.aliyun.com) | 需开通「通义万相」服务 |
| 智谱 | [open.bigmodel.cn](https://open.bigmodel.cn) | 需开通「CogVideoX」服务 |
| 可灵 | [klingai.com](https://klingai.com) | 需开通「可灵 API」服务 |
...
支持的音乐风格：cinematic（电影感）、upbeat（欢快）、calm（平静）、dramatic（戏剧性）、romantic（浪漫）、sad（悲伤）、mysterious（神秘）、energetic（活力）、peaceful（宁静）、epic（史诗）
...
| api_key | string | API 密钥，支持 `${ENV_VAR}` 格式 |
| format | string | API 格式（volcengine/aliyun/zhipu/openai/kling） |
| model | string | 模型标识符 |
...
在 config.toml 中添加新的模型配置段：
```toml
[models.model5]
name = "新模型"
api_key = "your-api-key"
format = "volcengine"  # 支持: volcengine/aliyun/zhipu/openai/kling
model = "model-name"
...
### 2.1.2
- 新增可灵 Kling 服务商
- 新增火山引擎豆包视频生成模型 doubao-seedance-2-0 适配
- 修复部分参数传递不正确等已知 BUG
...
- [MaiBot](https://github.com/MaiM-with-u/MaiBot) - 优秀的 AI 机器人框架
