"""火山引擎视频生成服务商 - HTTP 实现"""

import logging
from typing import Dict, Any, Optional, Tuple

from .base import BaseProvider
from .capabilities import VOLCENGINE_CAPABILITIES
from ..core.http_client import AsyncHttpClient, HttpError


class VolcengineProvider(BaseProvider):
    """火山引擎视频生成服务商"""

    PROVIDER_NAME = "volcengine"
    CAPABILITIES = VOLCENGINE_CAPABILITIES

    ENDPOINT_CREATE = "/contents/generations/tasks"
    ENDPOINT_GET = "/contents/generations/tasks/{task_id}"
    ENDPOINT_DELETE = "/contents/generations/tasks/{task_id}"

    def __init__(
        self,
        api_key: str,
        base_url: str = "",
        logger: Optional[logging.Logger] = None,
    ):
        super().__init__(api_key, base_url, logger)
        self._base_url = base_url or "https://ark.cn-beijing.volces.com/api/v3"
        self._client = AsyncHttpClient(
            base_url=self._base_url,
            api_key=api_key,
            timeout=60,
            logger=self._logger,
        )
        self._logger.info(f"[VolcengineProvider] 初始化: {self._base_url}")

    @staticmethod
    def _normalize_image_url(url: Optional[str]) -> str:
        """确保图片数据是火山引擎接受的格式（data URI 或公网 URL）"""
        if not url:
            return ""
        # 已是完整 data URI，直接返回
        if url.startswith("data:image/"):
            return url
        # 公网 URL，直接返回
        if url.startswith(("http://", "https://")):
            return url
        # 纯 base64 数据，补上 data URI 前缀
        if url.startswith("/9j/"):
            return f"data:image/jpeg;base64,{url}"
        elif url.startswith("iVBOR"):
            return f"data:image/png;base64,{url}"
        elif url.startswith("R0lGO"):
            return f"data:image/gif;base64,{url}"
        elif url.startswith("UklGR"):
            return f"data:image/webp;base64,{url}"
        # 默认当 png
        return f"data:image/png;base64,{url}"

    async def create_task(
        self,
        model: str,
        prompt: str,
        image_url: Optional[str] = None,
        last_frame_url: Optional[str] = None,
        audio_url: Optional[str] = None,
        resolution: str = "720p",
        duration: int = 5,
        fps: int = 24,
        **kwargs
    ) -> str:
        # 根据模型调整时长限制
        if "2-0" in model or "2.0" in model:
            max_duration = 15
        else:
            max_duration = 12
        
        min_duration = 4
        original_duration = duration
        
        if duration > max_duration:
            duration = max_duration
            self._logger.info(f"[VolcengineProvider] 时长已调整: {original_duration}s -> {duration}s")
        elif duration < min_duration:
            duration = min_duration
            self._logger.info(f"[VolcengineProvider] 时长已调整: {original_duration}s -> {duration}s")
        
        has_first = bool(image_url)
        has_last = bool(last_frame_url)
        validated = self.validate_params(
            model=model,
            duration=duration,
            resolution=resolution,
            fps=fps,
            has_first_frame=has_first,
            has_last_frame=has_last,
        )
        
        for warning in validated.get("warnings", []):
            self._logger.warning(f"[VolcengineProvider] {warning}")
        
        actual_duration = validated["duration"]
        
        if has_first and has_last:
            mode = "首尾帧图生视频"
        elif has_first:
            mode = "首帧图生视频"
        elif has_last:
            mode = "尾帧图生视频"
        else:
            mode = "文生视频"
        
        self._logger.info(f"[VolcengineProvider] 创建任务: model={model}, mode={mode}")
        self._logger.debug(f"[VolcengineProvider] prompt={prompt[:50]}...")

        content = []
        
        watermark = kwargs.get("watermark", False)
        generate_audio = kwargs.get("generate_audio", False)
        camera_fixed = kwargs.get("camera_fixed", True)
        
        prompt_parts = [prompt]
        prompt_parts.append(f"--duration {actual_duration}")
        prompt_parts.append(f"--watermark {'true' if watermark else 'false'}")
        
        if has_first or has_last:
            prompt_parts.append("--ratio adaptive")
        
        prompt_parts.append(f"--camerafixed {'true' if camera_fixed else 'false'}")
        
        prompt_with_params = " ".join(prompt_parts)
        
        content.append({
            "type": "text",
            "text": prompt_with_params
        })
        
        if has_first:
            normalized_first = self._normalize_image_url(image_url)
            self._logger.info(f"[VolcengineProvider] 首帧图片长度: {len(normalized_first)}, 前缀: {normalized_first[:20]}...")
            first_frame_content = {
                "type": "image_url",
                "image_url": {"url": normalized_first}
            }
            if has_last:
                first_frame_content["role"] = "first_frame"
            content.append(first_frame_content)
            self._logger.debug("[VolcengineProvider] 添加首帧图片")

        if has_last:
            normalized_last = self._normalize_image_url(last_frame_url)
            self._logger.info(f"[VolcengineProvider] 尾帧图片长度: {len(normalized_last)}, 前缀: {normalized_last[:20]}...")
            content.append({
                "type": "image_url",
                "image_url": {"url": normalized_last},
                "role": "last_frame"
            })
            self._logger.debug("[VolcengineProvider] 添加尾帧图片")

        request_body = {
            "model": model,
            "content": content,
        }
        
        if generate_audio:
            request_body["generate_audio"] = True

        self._logger.debug(f"[VolcengineProvider] 请求体: {request_body}")

        try:
            response = await self._client.post(self.ENDPOINT_CREATE, request_body)
            
            task_id = response.get("id", "")
            if not task_id:
                raise Exception(f"未返回任务ID: {response}")
            
            self._logger.info(f"[VolcengineProvider] 任务创建成功: {task_id}")
            return task_id
            
        except HttpError as e:
            self._logger.error(f"[VolcengineProvider] 创建任务失败: {e}")
            raise Exception(str(e))
        except Exception as e:
            self._logger.error(f"[VolcengineProvider] 创建任务异常: {e}")
            raise

    async def get_task_status(self, task_id: str) -> Dict[str, Any]:
        endpoint = self.ENDPOINT_GET.format(task_id=task_id)
        
        try:
            response = await self._client.get(endpoint)
            status = response.get("status", "unknown")
            
            result = {
                "status": status,
                "progress": 0,
                "video_url": "",
                "message": ""
            }
            
            if status == "succeeded":
                result["progress"] = 100
                content = response.get("content", {})
                if isinstance(content, dict):
                    result["video_url"] = content.get("video_url", "")
                self._logger.info(f"[VolcengineProvider] 任务完成: {task_id}")
                
            elif status == "running":
                result["progress"] = 50
            elif status == "queued":
                result["progress"] = 10
            elif status == "failed":
                error = response.get("error", {})
                if isinstance(error, dict):
                    result["message"] = error.get("message", "生成失败")
                else:
                    result["message"] = str(error) if error else "生成失败"
                self._logger.error(f"[VolcengineProvider] 任务失败: {task_id} - {result['message']}")
            elif status == "cancelled":
                result["message"] = "任务已取消"
            elif status == "expired":
                result["status"] = "failed"
                result["message"] = "任务已过期"

            return result
            
        except HttpError as e:
            self._logger.error(f"[VolcengineProvider] 查询状态失败: {e}")
            return {"status": "error", "progress": 0, "video_url": "", "message": str(e)}
        except Exception as e:
            self._logger.error(f"[VolcengineProvider] 查询状态异常: {e}")
            return {"status": "error", "progress": 0, "video_url": "", "message": str(e)}

    async def cancel_task(self, task_id: str) -> Tuple[bool, str]:
        endpoint = self.ENDPOINT_DELETE.format(task_id=task_id)
        
        try:
            await self._client.delete(endpoint)
            self._logger.info(f"[VolcengineProvider] 任务已取消: {task_id}")
            return True, "任务已取消"
        except HttpError as e:
            error_msg = str(e)
            if "running" in error_msg.lower():
                return False, "运行中的任务无法取消"
            self._logger.error(f"[VolcengineProvider] 取消失败: {e}")
            return False, f"取消失败: {error_msg}"
        except Exception as e:
            self._logger.error(f"[VolcengineProvider] 取消异常: {e}")
            return False, f"取消失败: {e}"